...
if (!isDedicated) then
{
	if (isNil ("horde_jumpmf_gvar_running_already")) exitWith
	{
		...
		horde_jumpmf_fnc_key_pressed = compile preProcessFileLineNumbers "flop\addons\jump\mechanics\detect_key_input.sqf";
		horde_jumpmf_fnc_stop_vault_spam = compile preProcessFileLineNumbers "flop\addons\jump\mechanics\stop_vault_spam.sqf";
		...
		[] execVM "flop\addons\jump\init\function_init.sqf";
		
		[] execVM "flop\addons\jump\init\player_init.sqf";
	};
};