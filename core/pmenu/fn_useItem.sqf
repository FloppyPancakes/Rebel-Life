...
"barg"
	...
	{
		pconstruct = ([_item,1] call life_fnc_classToVar);
		[pconstruct] execVM "flop\Scripts\flp_construct.sqf";
	...
	case (_item == "bcharge"):
	{
		...
		if (cursorTarget isKindOf "Building") then 
			{
				[] execVM "flop\Scripts\flp_breachingCharge.sqf";
				[false,"bcharge",1] call life_fnc_handleInv;
			} else {
				hint "You can only use Breaching Charges on buildings.";
			};
